...
% 
%     %% SEMI-SUPERVISED: FINDING WORDS PCA AND THEN SVM
%     wc_train_meaned = bsxfun(@minus, word_counts_train, mean(word_counts_train));     
%     wc_train_cov = (wc_train_meaned' * wc_train_meaned)./size(wc_train_meaned,1);
%     [coeff_train, latent, exp] = pcacov(wc_train_cov);
% 
%     wc_score_train = word_counts_train * coeff_train(:,1:570);
%     wc_score_test = word_counts_test * coeff_train(:,1:570);
% 
%     wc_model = fitcsvm(wc_score_train, labels_train);
...
wc_train_meaned = bsxfun(@minus, word_counts_processed, mean(word_counts_processed));

wc_train_cov = (wc_train_meaned' * wc_train_meaned)./size(wc_train_meaned,1);

[coeff_train, latent, exp] = pcacov(wc_train_cov);

wc_score_train = word_counts_processed * coeff_train(:,1:570);

wc_model = fitcsvm(wc_score_train, labels);

saved_pc = coeff_train(:,1:570);

save(['models_svm_pca' '.mat'], 'wc_model', 'c_removed', 'preprocess', 'saved_pc');